...
// Special implementation for Negroni, but could be used elsewhere.
func (m *JWTMiddleware) HandlerWithNextNoError(w http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
	m.CheckJWTNoError(w, r)
...
func (m *JWTMiddleware) CheckJWTNoError(w http.ResponseWriter, r *http.Request) error {
	// Use the specified token extractor to extract a token from the request
	token, err := m.Options.Extractor(r)
